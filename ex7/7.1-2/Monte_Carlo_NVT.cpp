...
//Read input informations
  ...
  cout << "Thermalization steps performed before measuring : " << thstep << endl;
  ...
  if( instant == 0 )
  {
    ...
    cout << "Total number of steps = " << nstep * nblk << endl << endl;
  ReadInput.close();
...
//measurement of g(r)
  igofr = 2; //0 and 1 are index of E and P, g(r) start at 2
  nbins = 100;
...
// distance i-j in pbc
     ...
     //cout<<igofr + trunc(dr/bin_size)<<endl;
/*
...
void Evaluate(int step)
{
  ...
  Epot.open("epot.instant.out", ios::app);
  Epot << step << setw(wd) << walker[iv] *1. / npart + vtail << endl;
  Epot.close();

  Pres.open("pres.instant.out", ios::app);
  Pres << step << setw(wd) << rho * temp + (walker[iw] + npart * ptail) / vol << endl;
  Pres.close();
}
